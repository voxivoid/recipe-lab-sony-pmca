...
| **centre** | select the recipe you are looking at — the camera keeps it. A message confirms it |
...
| **TRASH** | stage the factory look, then **centre** to select it |
...
| **PREVIEW** | you are only looking; press **centre** to select it |
...
- Some recipes push saturation further than the menu slider goes (the menu allows ±3, the camera accepts more). The
  ...
  recipe loses that extra punch. Select the recipe again in the app if that happens.
- The preview inside the app is temporary; closing the app removes it. Only what you *selected* stays.
- Built and tested on the A6000 with firmware 3.21. Other Sony bodies of the same generation probably keep these
  settings in the same place, but nobody has checked — compare what the chips show with your menus before selecting a recipe.
...
| Look not applied after selecting a recipe | Turn the camera off and on |
